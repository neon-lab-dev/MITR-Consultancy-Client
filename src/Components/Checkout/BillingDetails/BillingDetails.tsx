...
const BillingDetails = () => {
  ...
  const totalPriceBeforeGst =
    cartData && cartData?.reduce((acc, currVal) => acc + currVal.price, 0);
  ...
  const handleCheckout = async () => {
    try {
      ...
      const newCartData = cartData?.filter(
        (course: any) => !purchasedCourseIds.includes(course._id)
      );
      ...
      const keyData = await axios.get(
        "https://mitr-backend.vercel.app/api/v1/getkey"
      );
      ...
      const options = {
        ...
        callback_url:
          "https://mitr-backend.vercel.app/api/v1/paymentverification", // success URL
        prefill: {
          id: user?._id,
          name: user?.name,
          email: user?.email,
